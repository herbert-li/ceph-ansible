            ...
            try:
                if "schema is missing" not in error.message:
                    given = "[{}] Given value for {}: {}".format(
                        host, error.path[0], error.path[1])
                    display.error(given)
                else:
                    given = ""
...
def ceph_repository_choices(value):
    assert value in ['community', 'rhcs',
                     'dev'], "ceph_repository must be either 'community', 'rhcs' or 'dev'"  # noqa E501
...
def validate_rados_options(value):
    ...
    radosgw_address_block_given = notario_store["radosgw_address_block"] != "subnet"  # noqa E501
